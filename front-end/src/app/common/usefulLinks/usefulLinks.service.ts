...
export class UsefulLinksService {
  ...
  private async loadList(): Promise<void> {
    const usefulLinks: UsefulLink[] = await this.api.getResource('useful-links');
    ...
    if (options.search)
      filteredList = filteredList.filter(x =>
        options.search
          .split(' ')
          .every(searchTerm =>
            [x.name, x.url, x.audience].filter(f => f).some(f => f.toLowerCase().includes(searchTerm))
          )
      );
  ...
  async getById(linkId: string): Promise<UsefulLink> {
    return new UsefulLink(await this.api.getResource(['useful-links', linkId]));
  }
  ...
  async insert(usefulLink: UsefulLink): Promise<UsefulLink> {
    return new UsefulLink(await this.api.postResource('useful-links', { body: usefulLink }));
  }
  ...
  async update(usefulLink: UsefulLink): Promise<UsefulLink> {
    return new UsefulLink(await this.api.putResource(['useful-links', usefulLink.linkId], { body: usefulLink }));
  }
  ...
  async swapSort(usefulLinkA: UsefulLink, usefulLinkB: UsefulLink): Promise<void> {
    ...
    await this.api.patchResource(['useful-links', usefulLinkA.linkId], { body });
  ...
  async delete(usefulLink: UsefulLink): Promise<void> {
    await this.api.deleteResource(['useful-links', usefulLink.linkId]);
  }
}
