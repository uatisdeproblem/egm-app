...
export class DocumentTemplateComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    // @todo check vars
    this.variables = [
      { code: 'user', description: this.t._('DOCUMENT_TEMPLATE.VARIABLES.USER') },
      { code: 'country', description: this.t._('DOCUMENT_TEMPLATE.VARIABLES.COUNTRY') },
      { code: 'section', description: this.t._('DOCUMENT_TEMPLATE.VARIABLES.SECTION') },
      { code: 'spotType', description: this.t._('DOCUMENT_TEMPLATE.VARIABLES.SPOT_TYPE') },
      { code: 'spotId', description: this.t._('DOCUMENT_TEMPLATE.VARIABLES.SPOT_ID') },
      { code: 'price', description: this.t._('DOCUMENT_TEMPLATE.VARIABLES.PRICE') },
      { code: 'url', description: this.t._('DOCUMENT_TEMPLATE.VARIABLES.URL') }
    ];
  }
  ...
  async askAndDownloadTestDocumentWithCurrentTemplate(): Promise<void> {
    const doSend = async (): Promise<void> => {
      try {
        ...
        this.message.success('DOCUMENT_TEMPLATE.EMAIL_SENT');
      } catch (error) {
        this.message.error('DOCUMENT_TEMPLATE.BAD_TEMPLATE');
      } finally {
        this.loading.hide();
      }
    };

    const header = this.t._('DOCUMENT_TEMPLATE.TEST_TEMPLATE');
    const message = this.t._('DOCUMENT_TEMPLATE.TEST_TEMPLATE_I');
