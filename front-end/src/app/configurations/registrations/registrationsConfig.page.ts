...
export class RegistrationsConfigurationsPage implements OnInit {
  ...
  async addSpotType(): Promise<void> {
    ...
    const header = this.t._('CONFIGURATIONS.ADD_SPOT_TYPE');
    const inputs: any = [
      { name: 'name', type: 'text', placeholder: this.t._('CONFIGURATIONS.SPOT_TYPE_NAME') },
      {
        name: 'price',
        type: 'number',
        placeholder: this.t._('CONFIGURATIONS.SPOT_TYPE_PRICE', { currency: this.configurations.currency })
      }
    ];
    const buttons = [
